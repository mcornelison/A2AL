...
|---|---|---|---|
| 0.5.0 | 2026-06-11 | Header Fields + Vocabulary Expansion | Adds four **optional** routing-header fields — `thread=<id>` (stable conversation/topic-group key, distinct from `in-reply-to`), `status=` (overall-status token), `cc=` (informational recipients), `priority=P0..P3` (triage axis, distinct from `urgency`) — plus multi-value `in-reply-to`. All optional and channel-dependent. Vocabulary expansion: **+10 core terms** (`ack`, `endorse`, `supersede`, `superseded`, `heads-up`, `gate`, `spike`, `hotfix`, `PARTIAL`, `WITHDRAWN`) and a new **`azure` domain** (22 terms: Entra/MI/OBO/FIC/MSAL/JWT/SSO/aud identity, PHI/PII/BAA/DLP/ZDR/Purview/UAL compliance, Fabric/OneLake/lakehouse/RLS/CLS/SCD2 data-platform). Library 117 → 149 entries. All changes backward-compatible with 0.4.x. Driven by an evidence sweep of live `offices/*/inbox` agent traffic (DataWarehouse + MCP-Server). See `specs/A2A-Core.md` §3.2 and `library/azure.yaml`. |
...
| Version | Theme |
|---|---|
| 0.5.1 | Tokenization validator tooling (auto-populate `tokens: {claude, gpt, llama}` per entry) |
| 0.5.2 | Auto-harvested PR candidates from agent traffic |
| 0.5.3 | Library search/lookup CLI |
| 0.6.0 | Moltbook beta — cross-LLM validation (early data already arriving from shell-prompt's MoltBook field-test) |
| 0.6.x | Cross-platform skill installations (OpenAI Assistants, raw API, etc.) |
