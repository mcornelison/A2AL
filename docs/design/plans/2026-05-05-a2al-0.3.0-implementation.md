...
## Task 13: Add "Relationship to Google A2A" section to specs/A2A-Core.md

**Files:**
- Modify: `specs/A2A-Core.md` (append new top-level section after existing §8 Validation)

A2AL is payload-layer. Google's Agent2Agent (A2A) protocol is transport/lifecycle-layer. The two compose. This task documents the relationship explicitly so /0.3.0 ships positioned correctly within the broader A2A ecosystem.

- [ ] **Step 1: Append a new §9 to `specs/A2A-Core.md` (after the existing §8 Validation, before EOF) with this content:**
...
```
Expected: 9 (was 8 before this task — sections 1 through 9).

- [ ] **Step 3: Commit**

```bash
git add specs/A2A-Core.md
git commit -m "Add §9 'Relationship to Google A2A' to A2A-Core.md

A2AL is payload-layer; Google A2A is transport/lifecycle-layer. They
compose: an A2AL message fits as a structured-data Part within a
Google A2A Message or Artifact. Documents the layering explicitly
from /0.3.0 onward; an a2a-integration profile is planned for /0.4.0."
```

---

## Task 14: Rewrite specs/A2A-Rulebook.md
...
## Task 15: Rewrite specs/IMPLEMENTING.md
...
## Task 16: Rewrite examples/Markdown-to-A2AL.md
...
## Task 18: Rewrite top-level README.md
...
## Task 19: Light refresh of CONTRIBUTING.md
...
## Task 21: Claude Code skill and command for A2AL
...
Additional inputs addressed:
- Version history tracking (added 2026-05-05) → Task 20 (`VersionHistory.md`)
- Claude Code agent integration (added 2026-05-05) → Task 21 (`examples/ClaudeCode/`)
- Relationship to Google A2A (added 2026-05-05 mid-execution) → Task 13 (new §9 in `A2A-Core.md`) and updated README content in Task 18
